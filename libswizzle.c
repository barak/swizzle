...
static int (*real_mkdir_fn)(const char *, mode_t);
static int (*real_mkdirat_fn)(int, const char *, mode_t);
...
int mkdir(const char *pathname, mode_t mode) {
    ...
    RESOLVE_SYMBOL(real_mkdir_fn, "mkdir");
    result = real_mkdir_fn(actual, mode);
    free(rewritten);
    return result;
}
...
int mkdirat(int dirfd, const char *pathname, mode_t mode) {
    ...
    RESOLVE_SYMBOL(real_mkdirat_fn, "mkdirat");
    result = real_mkdirat_fn(actual_dirfd, actual, mode);
    free(rewritten);
    return result;
}
